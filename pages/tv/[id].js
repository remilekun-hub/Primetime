...
function Tv({ data }) {
  ...
  return (
    <div className="text- text-white flex flex-col items-center gap-y-5 md:gap-y-0 md:flex-row md:items-start lg:justify-center pt-4 lg:pt-6">
      <Head>
        <title>{`${data.name} | PrimeTime`}</title>
...
export const getServerSideProps = async (ctx) => {
  ...
  const movie = await fetch(
    `${baseURl}/tv/${id}?api_key=${TMDBKEY}&append_to_response=credits`
  );
